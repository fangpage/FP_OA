<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="FP_Message.Controller.msgtime" %>
...
protected override void View()
{
	...
	ViewBuilder.Append("<title>待发出的短消息</title>\r\n");
	...
	ViewBuilder.Append("              <td><span class=\"td-nav-title\">待发出的短消息</span></td>\r\n");
	ViewBuilder.Append("            </tr>\r\n");
	ViewBuilder.Append("          </tbody>\r\n");
	...
	ViewBuilder.Append("                <th style=\"width:130px;text-align:center;\">标题</th>\r\n");
	...
	ViewBuilder.Append("                <th style=\"width:140px;text-align:center;\">操作</th>\r\n");
	ViewBuilder.Append("              </tr>\r\n");
	ViewBuilder.Append("            </thead>\r\n");
	ViewBuilder.Append("            <tbody>\r\n");
	...
	ViewBuilder.Append("                    <i class=\"ico-send\" title=\"待发送\"></i>&nbsp;<a title=\"" + echo(item.content) + "\" class=\"subject\" rel=\"tooltip\" href=\"msgview.aspx?id=" + echo(item.id) + "\" data-original-title=\"" + echo(item.content) + "\">" + echo(item.subject) + "</a>\r\n");
	...
	ViewBuilder.Append("                  <input id=\"msgid\" name=\"id\" value=\"" + echo(item.id) + "\" type=\"hidden\">\r\n");
	ViewBuilder.Append("                  <a target=\"_self\" href=\"msgadd.aspx?id=" + echo(item.id) + "\"><span class=\"label label-info\">修改</span></a>\r\n");

	if (item.issend==0)
	{
	ViewBuilder.Append("                  <a id=\"cancel\" href=\"#\"><span class=\"label label-info\">取消发送</span></a>\r\n");
	}
	else
	{
	ViewBuilder.Append("                  <a id=\"continue\" href=\"#\"><span class=\"label label-info\">恢复发送</span></a>\r\n");
	}//end if
	ViewBuilder.Append("                </td>\r\n");
	ViewBuilder.Append("              </tr>\r\n");
	}//end loop
	...
	ViewBuilder.Append("        });\r\n");
	ViewBuilder.Append("        $('#cancel').live('click', function () {\r\n");
	ViewBuilder.Append("            window.confirm('您确定要取消发送吗？', function (ret) {\r\n");
	ViewBuilder.Append("                if (ret == true) {\r\n");
	ViewBuilder.Append("                    $(\"#action\").val(\"cancel\");\r\n");
	ViewBuilder.Append("                    $(\"#frmpost\").submit();\r\n");
	ViewBuilder.Append("                }\r\n");
	ViewBuilder.Append("            });\r\n");
	ViewBuilder.Append("        });\r\n");
	ViewBuilder.Append("        $('#continue').live('click', function () {\r\n");
	ViewBuilder.Append("            window.confirm('您确定要恢复发送吗？', function (ret) {\r\n");
	ViewBuilder.Append("                if (ret == true) {\r\n");
	...
	ViewBuilder.Append("                $(win).find(\".win_title .title\").html('提醒消息 -> 待发出的短消息')\r\n");
	ViewBuilder.Append("            }\r\n");
	ViewBuilder.Append("        }\r\n");
