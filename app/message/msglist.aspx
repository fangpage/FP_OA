<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="FP_Message.Controller.msglist" %>
...
protected override void View()
{
	...
	ViewBuilder.Append("<title>我收到的短消息</title>\r\n");
	...
	ViewBuilder.Append("              <td><span class=\"td-nav-title\">我收到的短消息</span></td>\r\n");
	ViewBuilder.Append("            </tr>\r\n");
	ViewBuilder.Append("          </tbody>\r\n");
	...
	ViewBuilder.Append("                <a class=\"btn\" id=\"btn_remind\" href=\"#\">已读</a>\r\n");
	ViewBuilder.Append("                <a class=\"btn\" style=\"margin-right:8px;\" id=\"btn_allremind\" href=\"#\">全部已读</a>\r\n");
	...
	ViewBuilder.Append("                <th style=\"width:80px;text-align:center;\">发送人</th>\r\n");
	ViewBuilder.Append("                <th style=\"text-align:center;\">内容</th>\r\n");
	ViewBuilder.Append("                <th style=\"width:140px;text-align:center;\">发送时间</th>\r\n");
	ViewBuilder.Append("                <th style=\"width:80px;text-align:center;\">状态</th>\r\n");
	ViewBuilder.Append("              </tr>\r\n");
	ViewBuilder.Append("            </thead>\r\n");
	ViewBuilder.Append("            <tbody>\r\n");

	loop__id=0;
	foreach(MessageInfo item in _msglist)
	{
	loop__id++;
	...
	if (item.subject!="")
	{
	ViewBuilder.Append("                  <a class=\"subject\" title=\"" + echo(item.subject) + "\" rel=\"tooltip\" data-original-title=\"" + echo(item.subject) + "\" href=\"msgview.aspx?id=" + echo(item.id) + "\">" + echo(item.subject) + "</a>\r\n");
	}//end if
	ViewBuilder.Append("                </td>\r\n");
	ViewBuilder.Append("                <td class=\"ellipsis\" title=\"" + echo(item.fromname) + "\" rel=\"tooltip\" data-original-title=\"" + echo(item.fromname) + "\" style=\"text-align:center;\">" + echo(item.fromname) + "</td>\r\n");
	ViewBuilder.Append("                <td class=\"ellipsis\"><a class=\"subject\" title=\"" + echo(item.content) + "\" rel=\"tooltip\" href=\"msgview.aspx?id=" + echo(item.id) + "\" data-original-title=\"" + echo(item.content) + "\">" + echo(item.content) + "</a></td>\r\n");
	...
	if (item.isread==1)
	{
	ViewBuilder.Append("                  <span class=\"label label-info\"><b>已阅读</b></span>\r\n");
	}
	else
	{
	ViewBuilder.Append("                  <span class=\"label label-important\"><b>未阅读</b></span>\r\n");
	}//end if
	ViewBuilder.Append("                </td>\r\n");
	ViewBuilder.Append("              </tr>\r\n");
	}//end loop
	...
	ViewBuilder.Append("                $(win).find(\".win_title .title\").html('提醒消息 -> 我收到的短消息')\r\n");
	ViewBuilder.Append("            }\r\n");
	ViewBuilder.Append("        }\r\n");
	...
	ViewBuilder.Append("      });\r\n");
	ViewBuilder.Append("      //批量取消提醒\r\n");
	ViewBuilder.Append("      $('#btn_remind').live('click', function () {\r\n");
	...
	ViewBuilder.Append("              window.confirm('您确定要批量标志所选短消息为已读吗？', function (ret) {\r\n");
	...
	ViewBuilder.Append("      });\r\n");
	ViewBuilder.Append("      //批量取消提醒\r\n");
	ViewBuilder.Append("      $('#btn_allremind').live('click', function () {\r\n");
	ViewBuilder.Append("          window.confirm('您确定要全部标记短消息为已读吗？', function (ret) {\r\n");
	ViewBuilder.Append("              if (ret == true) {\r\n");
	...
	ViewBuilder.Append("          });\r\n");
	ViewBuilder.Append("          if (select.length > 0) {\r\n");
	ViewBuilder.Append("              window.confirm('确定要批量删除所选的短消息吗？删除后将无法进恢复。', function (ret) {\r\n");
	ViewBuilder.Append("                  if (ret == true) {\r\n");
	...
	ViewBuilder.Append("          }\r\n");
	ViewBuilder.Append("      });\r\n");
	ViewBuilder.Append("      //全部删除\r\n");
	ViewBuilder.Append("      $('#btn_alldelete').live('click', function () {\r\n");
	ViewBuilder.Append("          window.confirm('确定要全部删除短消息吗？删除后将无法进恢复。', function (ret) {\r\n");
	ViewBuilder.Append("              if (ret == true) {\r\n");
