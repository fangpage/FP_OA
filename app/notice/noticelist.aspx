<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="FP_Notice.Controller.noticelist" %>
...
protected override void View()
{
	...
	ViewBuilder.Append("<title>通知公告</title>\r\n");
	...
	ViewBuilder.Append("        })\r\n");
	ViewBuilder.Append("        $(\"#btn_allread\").click(function () {\r\n");
	ViewBuilder.Append("            if (confirm(\"您确定全部标记为已读吗？\")) {\r\n");
	...
	ViewBuilder.Append("              <td><span class=\"td-nav-title\">通知公告</span></td>\r\n");
	ViewBuilder.Append("            </tr>\r\n");
	ViewBuilder.Append("          </tbody>\r\n");
	...
	ViewBuilder.Append("                <a class=\"btn\" id=\"btn_read\" href=\"#\">已读</a>\r\n");
	ViewBuilder.Append("                <a class=\"btn\" id=\"btn_allread\" href=\"#\">全部已读</a>\r\n");
	ViewBuilder.Append("            </div>\r\n");
	...
	ViewBuilder.Append("                <th style=\"width:150px;text-align:center;\">发布者</th>\r\n");
	...
	loop__id=0;
	foreach(NoticeInfo item in _noticelist)
	{
	loop__id++;
	...
	ViewBuilder.Append("                  <a class=\"subject\" style=\""+(item.istop==1?"color:#ff0000;":"")+"\" href=\"noticeview.aspx?id=" + echo(item.id) + "\">\r\n");

	if (FPUtils.IsContain(userid,item.viewuid))
	{
	...
	ViewBuilder.Append("                  " + echo(item.title) + "</a>\r\n");
	ViewBuilder.Append("                </td>\r\n");

	if (typelist.Count>0)
	{
	ViewBuilder.Append("<td style=\"text-align:center;width:60px;\">" + echo(item.typename) + "</td>\r\n");
	}//end if
	ViewBuilder.Append("                <td style=\"text-align:center;width:80px;\" title=\"" + echo(item.realname) + "\" class=\"ellipsis\">" + echo(item.realname) + "</td>\r\n");
	...
	ViewBuilder.Append("        });\r\n");
	ViewBuilder.Append("        $('#btn_read').live('click', function () {\r\n");
	...
	ViewBuilder.Append("                window.confirm('您确定要批量把所选公告标为已读吗？', function (ret) {\r\n");
	...
	ViewBuilder.Append("        $('#btn_allread').live('click', function () {\r\n");
	ViewBuilder.Append("            window.confirm('您确定要全部标为已读公告吗？', function (ret) {\r\n");
	ViewBuilder.Append("                if (ret == true) {\r\n");
	ViewBuilder.Append("                    $(\"#action\").val(\"allread\");\r\n");
	ViewBuilder.Append("                    $(\"#frmpost\").submit();\r\n");
	ViewBuilder.Append("                }\r\n");
