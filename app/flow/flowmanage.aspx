<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="FP_Flow.Controller.flowmanage" %>
...
protected override void View()
{
	...
	ViewBuilder.Append("<title>流程管理</title>\r\n");
	...
	ViewBuilder.Append("              <li style=\"background: url(" + echo(adminpath) + "statics/images/add.gif) 2px 6px no-repeat\"><a href=\"flowadd.aspx?" + echo(query) + "\">添加</a></li>\r\n");
	...
	ViewBuilder.Append("              <td>流程应用</td>\r\n");
	ViewBuilder.Append("              <td>流程类型</td>\r\n");
	ViewBuilder.Append("        	  <td width=\"50\">状态</td>\r\n");
	ViewBuilder.Append("              <td width=\"280\">操作</td>\r\n");
	ViewBuilder.Append("            </tr>\r\n");
	...
	ViewBuilder.Append("              <td align=\"center\"><a style=\"color: #1317fc\" href=\"flowadd.aspx?sortid=" + echo(sortid) + "&id=" + echo(item.id) + "\">" + echo(item.name) + "</a></td>\r\n");
	ViewBuilder.Append("              <td align=\"center\">" + echo(item.SortInfo.name) + " </td>\r\n");
	ViewBuilder.Append("              <td align=\"center\">" + echo(item.AppInfo.name) + " </td>\r\n");
	ViewBuilder.Append("              <td align=\"center\">\r\n");

	if (item.type==1)
	{
	ViewBuilder.Append("                  <span style=\"color:#1317fc\">自由流程</span>\r\n");
	}
	else if (item.type==2)
	{
	ViewBuilder.Append("                  <span style=\"color:#1317fc\">任务交办</span>\r\n");
	}
	else
	{
	ViewBuilder.Append("                  <span style=\"color:#ff0000\">固定流程</span>\r\n");
	}//end if
	ViewBuilder.Append("              </td>\r\n");
	ViewBuilder.Append("              <td align=\"center\">\r\n");
	...
	if (item.status==1)
	{
	ViewBuilder.Append("                  <span style=\"color:#1317fc\">开启</span>\r\n");
	}
	else
	{
	...
	ViewBuilder.Append("              </td>\r\n");
	ViewBuilder.Append("              <td>\r\n");
	ViewBuilder.Append("                  <a style=\"color: #1317fc\" href=\"flowadd.aspx?sortid=" + echo(sortid) + "&id=" + echo(item.id) + "\">流程属性</a>\r\n");
	ViewBuilder.Append("                  <a style=\"color: #1317fc\" href=\"flowflag.aspx?sortid=" + echo(sortid) + "&id=" + echo(item.id) + "\">流程权限</a>\r\n");

	if (item.type==1||item.type==2)
	{
	ViewBuilder.Append("                  <span style=\"color:#dbcccc\">步骤设计</span>\r\n");
	ViewBuilder.Append("                  <span style=\"color:#dbcccc\">流程视图</span>\r\n");
	}
	else
	{
	ViewBuilder.Append("                  <a style=\"color: #1317fc\" href=\"flowsteplist.aspx?sortid=" + echo(sortid) + "&flowid=" + echo(item.id) + "\">流程步骤</a>\r\n");
	ViewBuilder.Append("                  <a style=\"color: #1317fc\" href=\"flowdesign.aspx?id=" + echo(item.id) + "\" target=\"_blank\">流程视图</a>\r\n");
	}//end if

	if (item.formid>0)
	{
	ViewBuilder.Append("                  <a style=\"color: #1317fc\" href=\"" + echo(apppath) + "form/formdesign.aspx?id=" + echo(item.formid) + "\" target=\"_blank\">流程表单</a>\r\n");
	}
	else
	{
	ViewBuilder.Append("                  <span style=\"color:#dbcccc\">流程表单</span>\r\n");
	}//end if
	ViewBuilder.Append("              </td>\r\n");
	ViewBuilder.Append("            </tr>\r\n");
	}//end loop
	...
	ViewBuilder.Append("    </tr>\r\n");
	ViewBuilder.Append("    <tr>\r\n");
	ViewBuilder.Append("       <td align=\"left\">共有" + echo(pager.total) + "个流程，页次：" + echo(pager.pageindex) + "/" + echo(pager.pagecount) + "，" + echo(pager.pagesize) + "条每页</td>\r\n");
