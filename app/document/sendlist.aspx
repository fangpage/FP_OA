<%@ Page language="c#" AutoEventWireup="false" EnableViewState="false" Inherits="FP_Document.Controller.sendlist" %>
...
protected override void View()
{
	...
	ViewBuilder.Append("<title>发文管理</title>\r\n");
	...
	ViewBuilder.Append("              <td><span class=\"td-nav-title\">发文管理</span></td>\r\n");
	...
	ViewBuilder.Append("            <div class=\"btn-group\"><a id=\"outlineremove\" class=\"btn\"><i class=\"icon-remove\"></i>删除发文</a></div>\r\n");
	...
	ViewBuilder.Append("              <tr>\r\n");
	ViewBuilder.Append("                <th style=\"width:20px;\"><input type=\"checkbox\" value=\"0\" name=\"chkall\" id=\"chkall\"></th>\r\n");
	...
	ViewBuilder.Append("                <th style=\"width:140px;text-align:center;\">时间</th>\r\n");
	ViewBuilder.Append("                <th style=\"width:60px;text-align:center;\">状态</th>\r\n");
	ViewBuilder.Append("                <th style=\"width:150px;text-align:center;\">操作</th>\r\n");
	ViewBuilder.Append("              </tr>\r\n");
	ViewBuilder.Append("            </thead>\r\n");
	ViewBuilder.Append("            <tbody>\r\n");
	...
	ViewBuilder.Append("                <td class=\"checkbox-column\"><input value=\"" + echo(item.id) + "\" id=\"chkid_" + echo(item.id) + "\" type=\"checkbox\" name=\"chkid\"></td>\r\n");
	ViewBuilder.Append("                <td class=\"ellipsis\">\r\n");
	ViewBuilder.Append("                    <i class=\"ico-draft\"></i>\r\n");
	ViewBuilder.Append("                    <a class=\"subject\" target=\"_blank\" href=\"docview.aspx?id=" + echo(item.id) + "\">" + echo(item.title) + "</a>\r\n");
	ViewBuilder.Append("                </td>\r\n");
	ViewBuilder.Append("                <td class=\"ellipsis\" style=\"text-align:center;\"><a href=\"docview.aspx?id=" + echo(item.id) + "\">" + echo(item.sendorg) + "</a></td>\r\n");
	...
	if (item.status==1)
	{
	ViewBuilder.Append("                    <span class=\"label label-warning\">已发送</span>\r\n");
	}
	else if (item.status>=2)
	{
	ViewBuilder.Append("                    <span class=\"label label-info\">已签完</span>\r\n");
	}
	else
	{
	ViewBuilder.Append("                    <span class=\"label label-important\">未发送</span>\r\n");
	}//end if
	ViewBuilder.Append("                </td>\r\n");
	ViewBuilder.Append("                <td style=\"text-align:center;\">\r\n");
	...
	if (item.status==0)
	{
	ViewBuilder.Append("                    <a class=\"td-link-icon\" href=\"create.aspx?id=" + echo(item.id) + "\"><i class=\"icon-pencil\"></i>编辑</a>&nbsp;\r\n");
	ViewBuilder.Append("                    <span style=\"color:#dcdbdb\"><i class=\"icon-pencil\"></i>签收情况</span>\r\n");
	}
	else
	{
	ViewBuilder.Append("                    <a data=\"" + echo(item.id) + "\" class=\"workchange\" href=\"#\"><i class=\"icon-fan\"></i>转办</a>\r\n");
	ViewBuilder.Append("                    <a class=\"td-link-icon\" href=\"docview.aspx?id=" + echo(item.id) + "&action=deal\" target=\"_blank\"><i class=\"icon-pencil\"></i>签收情况</a>\r\n");
	}//end if
	ViewBuilder.Append("                </td>\r\n");
	ViewBuilder.Append("              </tr>\r\n");
	}//end loop
	...
	ViewBuilder.Append("          </table>\r\n");
	ViewBuilder.Append("          <div class=\"toolbar\">\r\n");
	ViewBuilder.Append("            <div class=\"btn-group\"></div>\r\n");
	ViewBuilder.Append("<div class=\"pull-right\">\r\n");
	ViewBuilder.Append("    <div style=\"float:left;display:inline-block;margin-right:8px;height:30px;line-height:30px;\">共" + echo(pager.total) + "条</div>\r\n");
	ViewBuilder.Append("    <div class=\"fpager\">\r\n");
	ViewBuilder.Append("        <div class=\"fpager-pages\">\r\n");
	...
	ViewBuilder.Append("        });\r\n");
	ViewBuilder.Append("        $('#outlineremove').live('click', function () {\r\n");
	...
	ViewBuilder.Append("                window.confirm('确定要批量删除所选公文吗？', function (ret) {\r\n");
	...
	ViewBuilder.Append("                $.notify({ type: 'warning', message: { text: '请选择删除的选项！', icon: 'icon-info' } }).show();\r\n");
	ViewBuilder.Append("            }\r\n");
	ViewBuilder.Append("        });\r\n");
	ViewBuilder.Append("        $('#news-grid a.delete.td-link-icon').click(function () {\r\n");
	ViewBuilder.Append("            var cid = $(this).attr(\"data\");\r\n");
	ViewBuilder.Append("            window.confirm('确定要删除这条数据吗?', function (ret) {\r\n");
	ViewBuilder.Append("                if (ret == false) return;\r\n");
	ViewBuilder.Append("                $(\"#action\").val(\"delete\");\r\n");
	ViewBuilder.Append("                $(\"#chkid_\" + cid).attr(\"checked\", true);\r\n");
	ViewBuilder.Append("                $(\"#frmpost\").submit();\r\n");
	ViewBuilder.Append("            });\r\n");
	ViewBuilder.Append("        });\r\n");
